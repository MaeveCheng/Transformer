...
def save_predictions_parquet(predictions: Dict, output_path: str):
    """Simple parquet export for predictions."""
    # Ensure all arrays are 1-dimensional
    preds = np.asarray(predictions['predictions']).flatten()
    probs = np.asarray(predictions['probabilities']).flatten()
...
def save_predictions_csv(predictions: Dict, output_path: str):
...
def save_predictions(predictions: Dict, output_path: str, format: str = 'parquet'):
    """Save predictions in specified format."""
    if format == 'csv':
        # Change extension to .csv if needed
        if output_path.endswith('.parquet'):
            output_path = output_path.replace('.parquet', '.csv')
        save_predictions_csv(predictions, output_path)
    else:
        save_predictions_parquet(predictions, output_path)
...
def handle_predict_mode(args, config):
    ...
    for file_path, output_path in zip(files_to_process, output_paths):
        ...
        # Save predictions in specified format
        output_format = getattr(args, 'output_format', 'parquet')
        save_predictions(results['predictions'], output_path, format=output_format)
