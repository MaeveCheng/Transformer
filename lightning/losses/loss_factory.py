...
def create_loss_function(binary_config):
    """
    Create combined loss function with multiple primary losses and optional distribution matching.
    
    Args:
        binary_config: BinaryClassificationConfig object containing loss configuration
        
    Returns:
        CombinedLoss instance with all configured components
        
    Raises:
        ValueError: If configuration is invalid
    """
    if binary_config is None:
        logger.warning("No binary classification config provided - using default BCE loss only")
