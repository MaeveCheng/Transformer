...
class OrderBookLightningModule(pl.LightningModule):
    ...
    def __init__(
        ...
        self.criterion = create_loss_function(binary_classification_config)
    ...
    def _setup_metrics(self):
        ...
        # Define core metrics that are fast to compute
    ...
    def _prepare_targets(self, targets: torch.Tensor) -> torch.Tensor:
        """Prepare targets for binary classification.
        ...
        # Convert continuous values to binary if needed
        if targets.dtype == torch.float:
    ...
    def _compute_loss(
        ...
        # Ensure logits are 1D for SigmoidDistributionLoss
        if logits.dim() > 1:
            logits = logits.squeeze(-1)
        ...
        # Store loss components for logging (using weighted values)
    ...
    def _shared_eval_step(self, batch: Dict[str, torch.Tensor], phase: str) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
        ...
        targets = batch.get('targets') or batch.get('labels')  # Support both 'targets' and 'labels' keys
    ...
    def training_step(self, batch: Dict[str, torch.Tensor], batch_idx: int) -> torch.Tensor:
        ...
        # Print feature information on first batch
    ...
    def _display_accumulated_metrics(self, batch_range: tuple, samples: int, phase: str, 
        ...
        if 'confusion_matrix' in computed_metrics:
            ...
            if cm.dim() == 2 and cm.shape == (2, 2):
                ...
                # Skip if confusion matrix has unexpected shape
                logger.info(f"WARNING: Unexpected confusion matrix shape: {cm.shape}")
                tn = fp = fn = tp = 0
            
            # Only log if we have valid confusion matrix values
            if cm.numel() >= 4:
                logger.info(f"\nConfusion Matrix:")
