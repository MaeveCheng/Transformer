...
class CPUWindowCreator:
    ...
    def __init__(
        ...
        return_threshold_buy: float = 0.0001,
        return_threshold_sell: float = -0.0001,
        classification_mode: str = 'binary',  # Add classification mode parameter
        **kwargs
    ):
        """
        ...
        Args:
            ...
            return_threshold_buy: Threshold for buy signal (ternary mode)
            return_threshold_sell: Threshold for sell signal (ternary mode)
            classification_mode: 'binary' or 'ternary' classification mode
        """
        ...
        if classification_mode not in ['binary', 'ternary']:
            raise ValueError(f"Invalid classification_mode: {classification_mode}. Must be 'binary' or 'ternary'")

        # Store thresholds for ternary classification
        self.return_threshold_buy = return_threshold_buy
        self.return_threshold_sell = return_threshold_sell

        # Validate thresholds to avoid overlapping classes
        if classification_mode == 'ternary' and return_threshold_buy <= return_threshold_sell:
            raise ValueError(f"Buy threshold ({return_threshold_buy}) must be greater than sell threshold ({return_threshold_sell})")
        ...
        logger.info(f"CPUWindowCreator initialized: "
                   ...
                   f"classification_mode={self.classification_mode}, "
                   f"RNG test value={test_random:.6f}")
        if self.classification_mode == 'ternary':
            logger.info(f"Ternary thresholds: buy={self.return_threshold_buy}, sell={self.return_threshold_sell}")
    ...
    def _calculate_targets_vectorized(
        ...
        if np.any(valid_positions_mask):
            ...
            if self.classification_mode == 'ternary':
                # ========== TERNARY CLASSIFICATION BASED ON RETURNS ==========
                # Calculate returns instead of just direction
                with np.errstate(divide='ignore', invalid='ignore'):
                    returns = avg_future_prices / current_prices - 1
                    returns = np.nan_to_num(returns, nan=0.0, posinf=0.0, neginf=0.0)

                # Use thresholds from initialization
                return_threshold_buy = self.return_threshold_buy
                return_threshold_sell = self.return_threshold_sell

                # Generate ternary targets: 0=Hold, 1=Buy, 2=Sell
                buy_mask = returns >= return_threshold_buy
                sell_mask = returns <= return_threshold_sell
                hold_mask = ~buy_mask & ~sell_mask

                # Update targets for valid positions
                targets[valid_pos_indices[buy_mask], 0] = 1.0  # Buy
                targets[valid_pos_indices[sell_mask], 0] = 2.0  # Sell
                targets[valid_pos_indices[hold_mask], 0] = 0.0  # Hold

                # PHASE 3: Enhanced logging for verification
                num_buys = np.sum(buy_mask)
                num_sells = np.sum(sell_mask)
                num_holds = np.sum(hold_mask)
                valid_total = len(valid_pos_indices)

                if valid_total > 0 and logger.isEnabledFor(logging.DEBUG):
                    logger.debug(f"[TERNARY] Target distribution - Hold: {num_holds}/{valid_total} ({100*num_holds/valid_total:.1f}%), "
                               f"Buy: {num_buys}/{valid_total} ({100*num_buys/valid_total:.1f}%), "
                               f"Sell: {num_sells}/{valid_total} ({100*num_sells/valid_total:.1f}%), "
                               f"Invalid: {num_invalid}/{batch_size}")
            else:
                # ========== BINARY CLASSIFICATION ==========
                # Generate targets: 1 for buy (price up), 0 for sell (price down)
                ...
                if valid_total > 0 and logger.isEnabledFor(logging.DEBUG):
                    logger.debug(f"[BINARY] Target distribution - Buy: {num_buys}/{valid_total} ({100*num_buys/valid_total:.1f}%), "
                               f"Sell: {num_sells}/{valid_total} ({100*num_sells/valid_total:.1f}%), "
