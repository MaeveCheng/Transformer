...
class OrderBookTransformer(nn.Module):
    def __init__(self, config: Optional[Dict] = None, binary_classification_config=None):
        ...
        # Use n_features from config and store as instance variable
        if config.n_features is None:
            raise ValueError("n_features must be configured")
        self.input_features = config.n_features
    ...
    def forward(self, x: torch.Tensor, 
        ...
        assert predictions.shape == (batch_size, 1), (
            f"Predictions shape {predictions.shape} != expected ({batch_size}, 1)"
        )
...
class ClassificationHead(nn.Module):
    """
    Classification head for binary prediction using attention-weighted temporal aggregation.
    ...
    def __init__(self, d_model: int, dropout: float = 0.1):
        ...
        # Single linear layer for classification (standard transformer practice)
        self.fc = nn.Linear(d_model, 1, bias=False)  # Direct projection to logit
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Single linear projection to logit (standard transformer practice)
        logits = self.fc(x)  # [batch_size, 1]
        assert logits.shape == (batch_size, 1), f"Logits shape {logits.shape} != expected ({batch_size}, 1)"
